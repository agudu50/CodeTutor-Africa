...
export const TestResultModal: React.FC<TestResultModalProps> = memo(({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-5 bg-slate-950/70 dark:bg-slate-950/80 backdrop-blur-xs animate-in fade-in duration-150">
      <div className="relative w-full max-w-2xl max-h-[90vh] rounded-3xl border border-slate-200 dark:border-slate-700/80 bg-white dark:bg-[#1e1e1e] shadow-2xl overflow-hidden flex flex-col animate-in zoom-in-95 duration-150 text-slate-900 dark:text-slate-200">
        {/* ═══════════════════════════════════════════════════════════════
            THEMED TERMINAL MODAL HEADER BAR
            ═══════════════════════════════════════════════════════════════ */}
        <div className="h-12 px-4 bg-slate-100/90 dark:bg-[#252526] border-b border-slate-200 dark:border-[#181818] flex items-center justify-between shrink-0 select-none gap-2">
          ...
          <div className="flex items-center gap-2.5 min-w-0">
            ...
            <div className="h-4 w-px bg-slate-300 dark:bg-[#333333] shrink-0 mx-0.5" />

            <div className="flex items-center gap-2 truncate">
              <div className="flex items-center gap-1.5 text-xs font-mono font-bold text-slate-800 dark:text-slate-100">
                <Terminal className="w-3.5 h-3.5 text-brand-600 dark:text-[#569cd6]" />
                <span className="truncate">codetutor-test-runner</span>
              </div>

              {isPassedAll !== undefined && (
                <span
                  className={`inline-flex items-center gap-1 text-[10px] font-mono font-bold px-2.5 py-0.5 rounded-full border shadow-xs ${
                    isPassedAll
                ...
                >
                  {isPassedAll ? (
                    <>
                      <CheckCircle2 className="w-3 h-3 text-emerald-600 dark:text-[#4ec9b0]" />
                      <span>{passedCount}/{totalCount} Passed</span>
                    </>
                  ) : (
                    <>
                      <XCircle className="w-3 h-3 text-rose-600 dark:text-[#ff7b72]" />
          ...
          <div className="flex items-center gap-2 shrink-0">
            {runtimeMs !== undefined && (
              <span className="text-[11px] font-mono text-slate-700 dark:text-slate-300 flex items-center gap-1 bg-white dark:bg-[#333333] px-2 py-0.5 rounded-lg border border-slate-200 dark:border-slate-700 shadow-2xs">
            ...
            <button
              type="button"
              onClick={onClose}
              className="p-1.5 rounded-lg text-slate-400 hover:text-slate-900 dark:hover:text-white hover:bg-slate-200 dark:hover:bg-[#333333] transition-colors cursor-pointer"
        ...
        {/* ═══════════════════════════════════════════════════════════════
            THEMED MODAL BODY
            ═══════════════════════════════════════════════════════════════ */}
        <div className="p-5 overflow-y-auto space-y-4 flex-1 bg-slate-50/50 dark:bg-[#1e1e1e]">
          {/* AI Pedagogical Review Banner */}
          {feedback && (
            <div className="p-3.5 rounded-2xl bg-emerald-50/90 dark:bg-[#1b3a2a]/60 border border-emerald-200 dark:border-[#2e5d42] text-xs text-emerald-900 dark:text-slate-200 flex items-start gap-3 shadow-xs">
              ...
              <div className="space-y-0.5 flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[10px] font-mono font-medium px-1.5 py-0.2 rounded bg-emerald-100 dark:bg-[#2e5d42] text-emerald-800 dark:text-white">
          ...
          {activeCase && (
            <div className="rounded-2xl border border-slate-200 dark:border-slate-700/80 bg-white dark:bg-[#252526] overflow-hidden shadow-xs">
              {/* Case Header */}
              <div className="px-4 py-3 bg-slate-100/80 dark:bg-[#2d2d2d] border-b border-slate-200 dark:border-slate-700/80 flex items-center justify-between text-xs">
                <div className="flex items-center gap-2">
                ...
                >
                  {activeCase.passed ? (
                    <CheckCircle2 className="w-3.5 h-3.5 text-emerald-600 dark:text-[#4ec9b0]" />
                  ) : (
                    <XCircle className="w-3.5 h-3.5 text-rose-600 dark:text-[#ff7b72]" />
                  )}
              ...
              <div className="p-4 grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs font-mono">
                {/* Input Subcard */}
                <div className="p-3.5 rounded-xl bg-slate-50 dark:bg-[#1e1e1e] border border-slate-200 dark:border-slate-700/80 flex flex-col justify-between space-y-2.5">
                  ...
                  <code className="text-slate-900 dark:text-slate-100 font-bold block bg-white dark:bg-[#181818] p-3 rounded-lg border border-slate-200 dark:border-slate-800 select-all break-all text-xs sm:text-[13px] leading-relaxed">
                    {activeCase.input}
                  </code>
                </div>
                ...
                <div className="p-3.5 rounded-xl bg-slate-50 dark:bg-[#1e1e1e] border border-slate-200 dark:border-slate-700/80 flex flex-col justify-between space-y-2.5">
                  ...
                  <code className="text-emerald-700 dark:text-[#4ec9b0] font-bold block bg-emerald-50/50 dark:bg-[#181818] p-3 rounded-lg border border-emerald-200 dark:border-slate-800 select-all text-xs sm:text-[13px] leading-relaxed">
              ...
              <div className="px-4 py-2.5 bg-slate-100/60 dark:bg-[#181818] border-t border-slate-200 dark:border-slate-800 flex items-center justify-between text-[11px] text-slate-500 dark:text-slate-400 font-mono">
        ...
        {/* ═══════════════════════════════════════════════════════════════
            THEMED MODAL FOOTER
            ═══════════════════════════════════════════════════════════════ */}
        <div className="px-5 py-3.5 bg-slate-100/80 dark:bg-[#252526] border-t border-slate-200 dark:border-[#181818] flex items-center justify-between text-xs shrink-0">
          <span className="text-slate-600 dark:text-slate-400 font-mono text-[11px]">
            Air-Gapped Test Suite • Verified in {runtimeMs || 42} ms
          </span>
          <div className="flex items-center gap-2">
            {isPassedAll && onNextProblem && (
              <Button
                ...
                className="h-8 font-bold bg-brand-600 dark:bg-[#007acc] hover:bg-brand-700 dark:hover:bg-[#0062a3] text-white shadow-xs"
            ...
            <Button
              ...
              size="sm"
              onClick={onClose}
              className="h-8 font-semibold bg-white dark:bg-[#333333] hover:bg-slate-100 dark:hover:bg-[#3d3d3d] text-slate-700 dark:text-slate-200 border-slate-300 dark:border-slate-700"
            >
              Back to Code
            </Button>
          </div>
