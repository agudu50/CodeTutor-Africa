...
export const CodeBlock: React.FC<CodeBlockProps> = memo(({ code, language = 'text', caption }) => {
  ...
  return (
    <div className="rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-950 overflow-hidden my-3 shadow-xs">
      {/* Code Header Bar */}
      <div className="flex items-center justify-between px-4 py-2 bg-slate-900 border-b border-slate-800 text-xs select-none">
        <div className="flex items-center gap-2.5">
          ...
          <div className="flex items-center gap-1.5 text-slate-300 font-mono text-[11px] font-semibold uppercase">
        ...
        <button
          type="button"
          onClick={handleCopy}
          className="flex items-center gap-1.5 text-[11px] font-mono font-medium text-slate-400 hover:text-slate-200 bg-slate-800/60 hover:bg-slate-800 border border-slate-700/80 transition-colors py-1 px-2.5 rounded-md"
      ...
      {/* Code Content */}
      <div className="p-4 overflow-x-auto text-xs sm:text-sm font-mono leading-relaxed text-slate-100 selection:bg-brand-600/40">
